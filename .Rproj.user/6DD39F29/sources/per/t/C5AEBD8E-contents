...
### --- http://dx.doi.org/10.1145/2827872 (data info)
### --------------------------------------------------
...
UserSim <- as.matrix(read.csv('UserSim.csv',
                                  header = T))
diag(UserSim) <- 0
ItemSim <- as.matrix(read.csv('ItemSim.csv',
                                  header = T))
diag(ItemSim) <- 0
...
for (i in 1:length(ratingsData$Rating)) {
  ...
  ### --- Part 2: use UserSim matrix
  # - most similar users from UserSim
  simUsers <- order(UserSim[uID, ], decreasing = T)
  ...
  simUserSimilarities <- UserSim[uID, ][simUsers]
  # - store outputs 2A, 2B
  ...
  ### --- Part 4: use ItemSim matrix
  # - most similar items from ItemSim
  simItems <- order(ItemSim[mID, ], decreasing = T)
  ...
  simItemsSimilarities <- ItemSim[mID, ][simItems]
  # - store outputs 4A, 4B
